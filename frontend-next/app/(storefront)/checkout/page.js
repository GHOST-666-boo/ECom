...
/**
 * CheckoutPageContent Component (Next.js adapted)
 * 
 * Renders the checkout flow with address list selectors, payment gateways,
 * and integration with Razorpay SDK script loaders.
 */
  ...
  const handleRazorpayPayment = async (orderId) => {
    try {
      ...
      const options = {
        ...
        handler: () => { clearCart(); router.push(`/orders/${orderId}`); },
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (paymentMethod === 'cod') { clearCart(); router.push(`/orders/${order.id}`); }
