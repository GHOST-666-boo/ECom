...
/**
 * HomePageContent - Metallic Vriddhi (Oat Edition)
 * Editorial split hero, bento featured grid, product gallery, and newsletter CTA.
 */
function HomePageContent() {
  ...
  useEffect(() => {
    if (searchParams.get('loginRequired')) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-6" style={{ minHeight: '640px' }}>
            {(() => {
              ...
              return slotOrder.map(key => {
                ...
                const elementProps = slot.computed_link ? { href: slot.computed_link } : {};
...
export default function HomePage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center" style={{ background: '#fcf9f8' }}><span className="text-xl italic animate-pulse">Loading collection...</span></div>}>
      <HomePageContent />
    </Suspense>
  );
}
