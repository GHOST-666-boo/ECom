...
/**
 * ProductDetailPageClient Component (Next.js adapted)
 * 
 * Implements the interactive elements of the product detail page:
 * - Image gallery with swipe gestures, next/prev navigation
 * - Natural aspect ratio height calculation
 * - Watermarked image display
 * - Full screen Lightbox modal with keyboard controls
 * - Quantity increments and Add to Cart action
 * - Related products gallery
 */
export default function ProductDetailPageClient({ slug }) {
  ...
  const handleAddToCart = async () => {
    if (!isAuthenticated) { router.push('/?loginRequired=true'); return; }
  ...
  return (
    <div style={{ background: '#fcf9f8' }} className="pb-24">
      ...
      {isLightboxOpen && images[selectedImage] && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12 bg-black/90 backdrop-blur-md transition-opacity duration-300"
          onClick={() => setIsLightboxOpen(false)}
        >
          <button
            className="absolute top-6 right-6 text-white/70 hover:text-white text-3xl font-light transition-all p-3 z-[120] hover:scale-110"
            onClick={() => setIsLightboxOpen(false)}
          >
            ✕
          </button>
          ...
          {images.length > 1 && (
            <button
              className="absolute left-6 text-white/50 hover:text-white text-5xl font-light transition-all p-4 z-[120] hover:scale-110 active:scale-95"
              onClick={(e) => {
                e.stopPropagation();
                setSelectedImage(prev => (prev === 0 ? images.length - 1 : prev - 1));
              }}
          ...
          {images.length > 1 && (
            <button
              className="absolute right-6 text-white/50 hover:text-white text-5xl font-light transition-all p-4 z-[120] hover:scale-110 active:scale-95"
              onClick={(e) => {
                e.stopPropagation();
                setSelectedImage(prev => (prev === images.length - 1 ? 0 : prev + 1));
              }}
          ...
          <div
            className="relative max-w-full max-h-[85vh] flex flex-col items-center justify-center mb-16"
            onClick={e => e.stopPropagation()}
          >
